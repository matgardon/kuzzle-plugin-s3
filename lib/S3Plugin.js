...
class S3Plugin {
  ...
  async bucketCreate (request) {
    ...
    const
      bucketName = this._stringArg(request, 'bucketName', this.config.bucketName),
      bucketRegion = this._stringArg(request, 'bucketRegion', this.config.region),
    ...
    if (bucketPolicy) {
      await this.s3.putBucketPolicies({...params, Policy: JSON.stringify(bucketPolicy)}).promise();
    }
  ...
  async bucketDelete (request) {
    ...
    const bucketRegion = this._stringArg(request, 'bucketRegion', this.config.bucketRegion);

    AWS.config.update({ region: bucketRegion });

    const params = { Bucket: bucketName };

    this.s3.deleteBucket(params).promise();
